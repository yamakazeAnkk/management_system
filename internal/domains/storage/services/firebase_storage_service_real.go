...
import (
	...
	"management_system/internal/domains/storage/interfaces"
...
func (s *FirebaseStorageServiceReal) UploadFile(ctx context.Context, req types.FileUploadRequest) (*types.FileUploadResponse, error) {
...
func (s *FirebaseStorageServiceReal) UploadAvatar(ctx context.Context, file multipart.File, header *multipart.FileHeader, userID string) (*types.FileUploadResponse, error) {
	return s.UploadFile(ctx, types.FileUploadRequest{
		File:        file,
		Header:      header,
		Folder:      "avatars",
		UserID:      userID,
...
func (s *FirebaseStorageServiceReal) UploadDocument(ctx context.Context, file multipart.File, header *multipart.FileHeader, userID, documentType string) (*types.FileUploadResponse, error) {
	return s.UploadFile(ctx, types.FileUploadRequest{
		File:        file,
		Header:      header,
		Folder:      "documents",
...
func (s *FirebaseStorageServiceReal) GetFileInfo(ctx context.Context, fileURL string) (*types.FileUploadResponse, error) {
	// Extract object name from URL
	parts := strings.Split(fileURL, "/")
	if len(parts) < 4 {
